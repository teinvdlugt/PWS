...
dialogue_file = "./subtitles/os/bigfile.txt"
train_file = "./subtitles/os/train.txt"
test_file = "./subtitles/os/test.txt"
unzipped = "./subtitles/os/unzipped/"
...
def process_zipped_xml_file(file_path):
    ...
    with gzip.open(file_path, "r") as f:
        for line in f:
            line = line.strip()
            if line.startswith('<') or len(line) == 0 or line.startswith('[') or line.endswith(':'):
...
def process_data():
    zipped_file = maybe_download("./subtitles/os/", "opensub2016.tar.gz", dialogue_url)
    unzipped_path = maybe_unzip(zipped_file, unzipped)
...
def get_data():
    if not (os.path.exists(train_file) and os.path.exists(test_file)):
        ...
        split_data(dialogue_file, train_file, test_file)
        pass
    else:
